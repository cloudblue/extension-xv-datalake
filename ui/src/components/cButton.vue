<template>
  ...
  >
    ...
    <div
      v-if="$slots['right-icon'] || iconRight"
...
const contrastHexColor = v => (isBright(v) ? '#000000' : '#FFFFFF');
...
export default {
  ...
  computed: {
    ...
    computedColor: vm => cond([
      ...
      [prop('color'), pipe(prop('color'), colorToRGB, rgbToStyleVar('button-computed-color'))],
      [T, pipe(prop('initialBtnColor'), colorToRGB, rgbToStyleVar('button-color'))],
    ])(vm),
    ...
    computedContentColor: ifElse(
      ...
      pipe(
        prop('buttonColor'),
        toContrastRGB,
        rgbToStyleVar('solid-content-color'),
      ),
    ),
